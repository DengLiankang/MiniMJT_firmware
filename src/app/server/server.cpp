...
static void server_process(AppController *sys, const ImuAction *action)
{
    ...
    if (0 == run_data->web_start && 0 == run_data->req_sent) {
        // 预显示
        display_setting("WebServer Start", "Domain: holocubic", "Wait...", "Wait...",
                        // "", "",
...
static void server_message_handle(const char *from, const char *to, APP_MESSAGE_TYPE type, void *message,
                                  void *ext_info)
{
    switch (type) {
...
APP_OBJ server_app = {
    SERVER_APP_NAME,      &app_server,          "", server_init, server_process, server_background_task, NULL,
