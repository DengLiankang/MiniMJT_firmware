...
static bool video_start(bool create_new)
{
    ...
    } else if (NULL != strstr(run_data->pfile->file_name, ".rgb") ||
               NULL != strstr(run_data->pfile->file_name, ".RGB")) {
        // 使用RGB格式的视频
...
static void media_player_message_handle(const char *from, const char *to, APP_MESSAGE_TYPE type, void *message,
                                        void *ext_info)
{
    switch (type) {
