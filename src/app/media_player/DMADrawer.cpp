...
static void lcd_spi_post_transfer_callback(spi_transaction_t *t) { gpio_set_level(tft_cs_pin, 1); }

static spi_device_handle_t spi_start(uint32_t len, int spi_freq, int tft_mosi, int tft_miso, int tft_sclk, int tft_cs,
                                     int tft_dc)
{
    ...
    spi_bus_config_t buscfg = {.mosi_io_num = (gpio_num_t)tft_mosi,
...
uint16_t *DMADrawer::getNextBuffer() { return _pixBuf[_pixFlip]; }
